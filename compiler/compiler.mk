...
PSS_COMPILER_DIR := $(dir $(lastword $(MAKEFILE_LIST)))
PSS_COMPILER_SRC_DIR := $(abspath $(PSS_COMPILER_DIR)/src)
...
ANTLR_PATCH_FILE = $(PSS_COMPILER_SRC_DIR)/../antlr4-cpp-runtime-$(ANTLR_VERSION)-source.patch

PSS_COMPILER_SRC = $(notdir $(wildcard $(PSS_COMPILER_SRC_DIR)/*.cpp))
...
PSS_COMPILER_LINK = -L. -lpss_compiler
else
ANTLR4_LINK = -lantlr/antlr_runtime
PSS_COMPILER_LINK = -lpss_compiler
endif

PACKAGES += $(PACKAGES_DIR)/$(ANTLR_JAR)
PACKAGES += $(PACKAGES_DIR)/$(ANTLR_RUNTIME_SRC_ZIP)
PACKAGES += $(PACKAGES_DIR)/$(ANTLR_RUNTIME_WIN_ZIP)
...
pss-grammar.gen : $(PSS_COMPILER_SRC_DIR)/PSS.g4 $(PACKAGES_DIR)/$(ANTLR_JAR)
	$(Q)mkdir -p grammar
	$(Q)java -jar $(PACKAGES_DIR)/$(ANTLR_JAR) \
		-Dlanguage=Cpp -visitor -o grammar $(PSS_COMPILER_SRC_DIR)/PSS.g4
	$(Q)touch $@

expr-grammar.gen : $(PSS_COMPILER_SRC_DIR)/Expr.g4 $(PACKAGES_DIR)/$(ANTLR_JAR)
	$(Q)mkdir -p expr_grammar
	$(Q)java -jar $(PACKAGES_DIR)/$(ANTLR_JAR) \
		-Dlanguage=Cpp -visitor -o expr_grammar \
		$(PSS_COMPILER_SRC_DIR)/Expr.g4
	$(Q)touch $@
